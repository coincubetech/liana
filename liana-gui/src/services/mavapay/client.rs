...
impl MavapayClient {
    ...
    pub fn new(api_key: String) -> Self {
        ...
        if api_key.is_empty() {
            ...
            tracing::info!("Mavapay client initialized with API key: {}...", &api_key[..api_key.len().min(8)]);
        }
    ...
    fn request(&self, method: Method, endpoint: &str) -> RequestBuilder {
        ...
        tracing::debug!("Mavapay API request: {} {} with API key: {}...",
            method, url, &self.api_key[..self.api_key.len().min(8)]);
    ...
    pub async fn get_price(&self, currency: &str) -> Result<PriceResponse, MavapayError> {
        ...
        tracing::debug!("Price API response: {}", serde_json::to_string_pretty(&price_data).unwrap_or_default());
        ...
        let price = if let Some(btc_price) = price_data.get("btcPriceInUnitCurrency").and_then(|p| p.as_str()) {
            // Primary: Handle the actual API response format at root level - btcPriceInUnitCurrency as string
            tracing::debug!("Found root-level btcPriceInUnitCurrency as string: {}", btc_price);
            btc_price.parse::<f64>().map_err(|_| {
                MavapayError::InvalidResponse(format!("Cannot parse btcPriceInUnitCurrency '{}' as float", btc_price))
            })?
        } else if let Some(btc_price) = price_data.get("btcPriceInUnitCurrency").and_then(|p| p.as_f64()) {
            // Primary: Handle the actual API response format at root level - btcPriceInUnitCurrency as number
            tracing::debug!("Found root-level btcPriceInUnitCurrency as number: {}", btc_price);
            btc_price
        } else if let Some(unit_price) = price_data.get("unitPricePerSat").and_then(|p| p.get("amount")).and_then(|a| a.as_str()) {
            // Alternative: use unit price per sat at root level
            tracing::debug!("Found root-level unitPricePerSat amount as string: {}", unit_price);
            unit_price.parse::<f64>().map_err(|_| {
                MavapayError::InvalidResponse(format!("Cannot parse unitPricePerSat amount '{}' as float", unit_price))
            })?
            ...
            tracing::debug!("Found nested data structure: {}", serde_json::to_string_pretty(&data).unwrap_or_default());

            if let Some(btc_price) = data.get("btcPriceInUnitCurrency").and_then(|p| p.as_str()) {
                tracing::debug!("Found nested btcPriceInUnitCurrency as string: {}", btc_price);
                btc_price.parse::<f64>().map_err(|_| {
                    MavapayError::InvalidResponse(format!("Cannot parse nested btcPriceInUnitCurrency '{}' as float", btc_price))
                })?
            } else if let Some(btc_price) = data.get("btcPriceInUnitCurrency").and_then(|p| p.as_f64()) {
                tracing::debug!("Found nested btcPriceInUnitCurrency as number: {}", btc_price);
                btc_price
            } else if let Some(unit_price) = data.get("unitPricePerSat").and_then(|p| p.get("amount")).and_then(|a| a.as_str()) {
                tracing::debug!("Found nested unitPricePerSat amount as string: {}", unit_price);
                unit_price.parse::<f64>().map_err(|_| {
                    MavapayError::InvalidResponse(format!("Cannot parse nested unitPricePerSat amount '{}' as float", unit_price))
                })?
                ...
                tracing::error!("No price found in nested data. Available fields: {:?}", data.as_object().map(|o| o.keys().collect::<Vec<_>>()).unwrap_or_default());
                return Err(MavapayError::InvalidResponse(format!("No price found in nested data object: {}", data)));
            }
        } else {
            return Err(MavapayError::InvalidResponse(format!("No price found in response. Available fields: {:?}", price_data.as_object().map(|o| o.keys().collect::<Vec<_>>()).unwrap_or_default())));
        };
    ...
    pub async fn create_quote(&self, request: QuoteRequest) -> Result<QuoteResponse, MavapayError> {
        ...
        let api_response: ApiResponse<QuoteResponse> = response.json().await
            .map_err(|e| MavapayError::InvalidResponse(format!("Failed to parse quote response: {}", e)))?;
    ...
    /// Get wallet balance
    pub async fn get_wallet_balance(&self, currency: Option<&str>) -> Result<Vec<WalletBalance>, MavapayError> {
    ...
    pub async fn get_transactions(
        ...
        let transactions = tx_data
            ...
            .ok_or_else(|| MavapayError::InvalidResponse("Missing transactions data".to_string()))?;

        let mut tx_list = Vec::new();
        for tx in transactions {
            if let (Some(id), Some(amount), Some(currency), Some(status), Some(created), Some(updated)) = (
                ...
                tx_list.push(Transaction {
                    ...
                    hash: tx.get("hash").and_then(|h| h.as_str()).map(|s| s.to_string()),
                });
            }
        }
    ...
    pub async fn confirm_quote(&self, quote_id: String) -> Result<PaymentStatusResponse, MavapayError> {
        ...
        let api_response: ApiResponse<PaymentStatusResponse> = response.json().await
            .map_err(|e| MavapayError::InvalidResponse(format!("Failed to parse confirmation response: {}", e)))?;
    ...
    pub async fn get_payment_status(&self, quote_id: &str) -> Result<PaymentStatusResponse, MavapayError> {
        let response = self
        ...
        let api_response: ApiResponse<PaymentStatusResponse> = response.json().await
            .map_err(|e| MavapayError::InvalidResponse(format!("Failed to parse status response: {}", e)))?;
    ...
    pub async fn get_transaction(&self, transaction_id: &str) -> Result<Transaction, MavapayError> {
        ...
        let api_response: ApiResponse<Transaction> = response.json().await
            .map_err(|e| MavapayError::InvalidResponse(format!("Failed to parse transaction response: {}", e)))?;
    ...
    pub async fn register_webhook(&self, url: &str, secret: &str) -> Result<(), MavapayError> {
        ...
        let api_response: ApiResponse<serde_json::Value> = response.json().await
            .map_err(|e| MavapayError::InvalidResponse(format!("Failed to parse webhook response: {}", e)))?;

        if api_response.status != "ok" {
            return Err(MavapayError::InvalidResponse("Failed to register webhook".to_string()));
        }
    ...
    pub async fn poll_transaction_status(
        ...
        for attempt in 1..=max_attempts {
            tracing::debug!("Polling payment status, attempt {}/{}", attempt, max_attempts);
