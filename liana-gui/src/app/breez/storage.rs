...
fn get_lightning_wallet_dir(network_dir: &Path, wallet_checksum: &str) -> PathBuf {
    network_dir.join(wallet_checksum).join(LIGHTNING_SUBDIR)
}
...
pub fn store_lightning_mnemonic(
    ...
    fs::create_dir_all(&wallet_dir).map_err(|e| {
        BreezError::Config(format!(
            "Failed to create Lightning wallet directory: {}",
            e
        ))
    })?;
...
pub fn load_lightning_mnemonic(
    network_dir: &Path,
    wallet_checksum: &str,
...
mod tests {
    ...
    fn test_wallet_path_generation() {
        ...
        let expected =
            PathBuf::from("/home/user/.liana/bitcoin/abc123xyz/lightning/lightning_wallet.enc");
