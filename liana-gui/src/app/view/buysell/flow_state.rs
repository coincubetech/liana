...
impl MavapayState {
    ...
    pub fn open_payment_link(&self) -> Task<BuySellMessage> {
        ...
        let request = CreatePaymentLinkRequest {
            name: format!("Vault Wallet - {} Payment", settlement_currency),
            description: format!(
                "[COINCUBE] One-time payment of {} {}",
                mavapay_amount, settlement_currency
            ),
            _type: PaymentLinkType::OneTime,
            add_fee_to_total_cost: false,
            settlement_currency: settlement_currency.clone(),
            payment_methods: vec![payment_method.clone()],
            amount: *mavapay_amount,
